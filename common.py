...
COMMANDS = {
    ...
    'get': ('get <remote filename>',
            'Downloads a file with a given filename (use "get all" to '
            'retrieve all files)'),
    'put': ('put <local file path> <remote file name>',
...
def validate_file_md5_hash(file, original_hash):
    """ Returns true if file MD5 hash matches with the provided one,
    false otherwise. """
...
def clean_and_split_input(input):
    """ Removes carriage return and line feed characters and splits input on
    a single whitespace. """
