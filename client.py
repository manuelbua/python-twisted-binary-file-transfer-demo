...
from common import COMMANDS, validate_file_md5_hash, \
    get_file_md5_hash, read_bytes_from_file, clean_and_split_input
...
class CommandLineProtocol(basic.LineReceiver):
    ...
    def connectionMade(self):
        ...
        self.connection = reactor.connectTCP(self.server_ip, self.server_port,
                                             self.factory)
        self.factory.deferred.addCallback(self._display_response)

    def lineReceived(self, line):
        """ If a line is received, call sendCommand(), else prompt user for
        input. """
...
class FileTransferProtocol(basic.LineReceiver):
    ...
    def lineReceived(self, line):
        if line == 'ENDMSG':
            ...
            self.file_complete = 0
            self.file_data = (filename, file_hash, file_size)
            self.setRawMode()
        else:
    ...
    def rawDataReceived(self, data):
        ...
        file_size = self.file_data[2]
        self.file_complete += len(data)
        completed_at = (self.file_complete / float(file_size)) * 100

        print 'Receiving file chunk, completed at %s%% (%d KB/%d KB)\r' % (
            int(completed_at),
            self.file_complete / 1024,
            int(file_size) / 1024
        ),
        ...
        if data.endswith('\r\n'):
            ...
            if validate_file_md5_hash(file_path, self.file_data[1]):
                print 'File %s has been successfully transfered and saved!' % (
                    filename)
            else:
                os.unlink(file_path)
                print 'File %s has been successfully transfered, ' \
...
if __name__ == '__main__':
    ...
    parser.add_option('--ip', action='store', type='string', dest='ip_address',
                      default='127.0.0.1', help='server IP address')
    parser.add_option('-p', '--port', action='store', type='int', dest='port',
                      default=1234, help='server port')
    ...
    stdio.StandardIO(
        CommandLineProtocol(options.ip_address, options.port, options.path))
    reactor.run()
